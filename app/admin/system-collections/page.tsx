...
export default function AdminSystemCollectionsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const response = await api.collections.listPage({ kind: "system", authenticated: true, sort_by: sortBy, view_count: true, page: currentPage, page_size: itemsPerPage });
        if (cancelled) return;
        const rows = (response?.items || []) as Collection[];
        setCollections(rows || []);
